...
export const COMMAND_CENTER_V1_GATE4_CAPABILITIES: Readonly<Record<CommandCenterV1MobileCapabilityId, {
  ...
  pwa_installability_contract: {
    ...
    evidence: ['client/public/manifest.json', 'client/public/icons/icon-180x180.png', 'client/index.html', 'scripts/command-center-mobile-pwa-smoke.mjs'],
  },
  standalone_apple_metadata: {
    ...
    evidence: ['client/index.html', 'scripts/command-center-mobile-pwa-smoke.mjs'],
  },
  service_worker_update_lifecycle: {
    ...
    evidence: ['client/public/sw.js', 'scripts/command-center-mobile-pwa-smoke.mjs'],
  },
  private_api_network_only: {
    id: 'private_api_network_only',
    status: 'certified',
    reason: 'Same-origin /api requests are network-only with cache:no-store and the mobile smoke asserts no API request appears in CacheStorage.',
    evidence: ['client/public/sw.js', 'scripts/command-center-mobile-pwa-smoke.mjs'],
  },
  iphone_safe_area_shell: {
  ...
  iphone_viewports_375_390_430: {
    ...
    evidence: ['scripts/command-center-mobile-pwa-smoke.mjs'],
  },
  touch_target_geometry: {
    id: 'touch_target_geometry',
    status: 'certified',
    reason: 'Hamburger, drawer-close, and drawer navigation controls are hardened to at least 44px touch geometry and verified in the browser.',
    evidence: ['client/src/styles/mobilePwa.css', 'client/src/components/TiberLayout.tsx', 'scripts/command-center-mobile-pwa-smoke.mjs'],
  },
  form_focus_keyboard_geometry: {
    id: 'form_focus_keyboard_geometry',
    status: 'certified',
    reason: 'Phone form controls use at least 16px focused text to avoid Safari auto-zoom, safe scroll margins, and browser-verified visible focus geometry.',
    evidence: ['client/src/styles/mobilePwa.css', 'scripts/command-center-mobile-pwa-smoke.mjs'],
  },
  text_zoom_200_percent: {
  ...
  failed_network_fail_closed: {
    ...
    status: 'certified',
    reason: 'Offline API fetches must reject rather than replay private data; the app falls back to the public Draft Review capability boundary.',
    evidence: ['client/public/sw.js', 'client/src/App.tsx', 'scripts/command-center-mobile-pwa-smoke.mjs'],
  },
  deep_link_shell_recovery: {
    id: 'deep_link_shell_recovery',
    status: 'certified',
    reason: 'Deep-link documents use the shared SPA shell and the service worker can return a cached document shell without replaying API state.',
    evidence: ['server/index.ts', 'client/public/sw.js', 'scripts/command-center-mobile-pwa-smoke.mjs'],
  },
  mobile_large_list_performance: {
