...
| Devy prospect signal/trajectory | GOVERNED/PARTIAL UPSTREAM | TIBER-Rookies already owns age-adjusted production, breakout age, production scores, draft-capital proxies, lifecycle horizon/confidence/volatility and discovery pulses; FFCC should consume rather than recreate | foundation |
| **Devy carrying cost / arrival utility** | DECISION-LAYER GAP | multi-year roster opportunity cost of holding a pre-NFL asset, expected time to NFL liquidity, roster-slot scarcity and contender/rebuilder horizon using upstream uncertainty rather than a new prospect grade | **P2** |
| Transfer/role-context devy change | PARTIAL UPSTREAM | TIBER-Rookies tracks transfer/pathway/lifecycle context and roster pulses; downstream causal effect on arrival/role utility remains a research consumer problem | P3 |
...
- **Rookie/devy route and draft evidence:** TIBER-Rookies already owns route profiles, college production, age-adjusted production, breakout-age computation, draft-capital/result evidence, development-horizon/lifecycle signals, volatility/confidence bands, roster pulses and league-market snapshot tooling. FFCC should consume promoted/eligible artifacts and must not recreate that producer logic. The remaining gap is downstream multi-year asset utility/carrying cost, not another prospect score.
