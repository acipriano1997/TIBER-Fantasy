...
function featureEvidence(
  ...
  for (const key of featureKeys) {
    ...
    values[key] = value;
    if (feature.sourceRefs.length === 0) {
      throw new CCFRoleBaselineModelError(
        `required feature ${key} must carry at least one source reference`,
      );
    }
    for (const ref of feature.sourceRefs) sourceRefs.add(requireText("feature sourceRef", ref));
    provenance.push({
      feature: key,
