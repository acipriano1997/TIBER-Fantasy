import { ccfRollingOpportunityWindowKeysV2 } from "../../features/rollingOpportunityFeatures";
...
const ROLLING_KEYS = ccfRollingOpportunityWindowKeysV2(3);
const TARGETS = ROLLING_KEYS.targetsMean;
const TARGET_SHARE = ROLLING_KEYS.targetShareMean;
...
describe("CCF Player Outcome Engine v0 inference kernel", () => {
  ...
  it("fails closed when a required model feature is unavailable rather than imputing it", () => {
    ...
    ).toThrow(`required model feature ${TARGET_SHARE} is missing`);
  });
