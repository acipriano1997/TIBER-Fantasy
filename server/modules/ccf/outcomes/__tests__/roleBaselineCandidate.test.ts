...
} from "../../features/rollingOpportunityFeatures";
...
const TARGETS = CCF_ROLLING_OPPORTUNITY_FEATURE_KEYS_V1.targetsPerRecordedGame;
const CARRIES = CCF_ROLLING_OPPORTUNITY_FEATURE_KEYS_V1.carriesPerRecordedGame;
const TARGET_SHARE = CCF_ROLLING_OPPORTUNITY_FEATURE_KEYS_V1.meanTargetShare;
...
describe("CCF role baseline outcome candidate", () => {
  it("builds a deterministic native candidate from canonical rolling role features", () => {
    const input = {
    ...
    expect(first.usedFeatureKeys).toEqual([TARGETS, CARRIES, TARGET_SHARE]);
    ...
    expect(first.outcome.mechanismContributions[0].evidenceRefs).toEqual(
      expect.arrayContaining([
        ...
        "ccf://rolling-opportunity-feature-receipt-v1",
