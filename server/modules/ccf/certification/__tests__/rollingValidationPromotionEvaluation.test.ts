...
function protocol(
  ...
  return {
    ...
    protocolId: "validation-promotion-evaluation-v1",
    ...
    samplePolicy: {
      ...
      underpoweredSubgroupTreatment: treatment,
    },
    uncertaintyPolicy: {
      ...
      deterministicSeed: "validation-promotion-evaluation",
...
function execution(
  ...
  const bindingId = "ccf://rolling-replay-binding/sha256/evaluation";
  const rows = [
    {
      rowId: "wr-1",
      blockId: "2024-W3",
...
function evidencePacket(
  ...
  const baselineEvidence = buildCCFRollingValidationBaselineEvidence({
    execution: run,
    protocol: frozen,
    baselinePredictions,
  });
  const overallEvidence = buildCCFRollingValidationPromotionEvidence({
    execution: run,
    baselineEvidence,
    baselinePredictions,
    protocol: frozen,
  ...
  const subgroupPromotionEvidence =
    buildCCFRollingValidationSubgroupPromotionEvidence({
      execution: run,
      protocol: frozen,
      baselinePredictions,
  ...
  return {
    run,
    overallEvidence,
    subgroupPromotionEvidence,
  };
}

describe("CCF rolling validation promotion evaluation", () => {
  it("binds subgroup evidence into the frozen criterion and evaluates validation criteria without opening the final holdout", () => {
    ...
    expect(
      overallEvidence.criterionEvidence[0].criterion.supportedSubgroupsPassed,
    ).toBeNull();

    const result = buildCCFRollingValidationPromotionEvaluation({
      protocol: frozen,
      overallEvidence,
      subgroupEvidence: subgroupPromotionEvidence,
    });

    expect(result).toMatchObject({
      validationCriteriaEvaluated: true,
      validationCriteriaPassed: true,
      finalHoldoutAccessed: false,
    ...
    expect(result.boundCriterionEvidence).toHaveLength(1);
    expect(result.boundCriterionEvidence[0].criterion).toMatchObject({
      criterionId: "fantasy-mae-vs-usage",
      supportedSubgroupsPassed: true,
    });
    expect(result.boundCriterionEvidence[0].subgroupCriterionEvidenceRef).toBe(
      subgroupPromotionEvidence.criterionEvidence[0].evidenceRef,
    );
    expect(result.boundCriterionEvidence[0].evidenceRef).toMatch(
      /^ccf:\/\/rolling-validation-bound-criterion-evidence\/sha256\/[a-f0-9]{64}$/,
    );
    expect(
      result.evaluation.criterionResults[0].subgroupGatePassed,
    ).toBe(true);
  });

  it("fails validation criteria when frozen report-only treatment leaves subgroup evidence non-passing", () => {
    ...
    expect(
      subgroupPromotionEvidence.criterionEvidence[0]
        .supportedSubgroupsPassed,
    ).toBe(false);
    expect(result.validationCriteriaPassed).toBe(false);
    expect(
      result.evaluation.criterionResults[0].subgroupGatePassed,
    ).toBe(false);
    expect(result.productionCertificationAuthorized).toBe(false);
  });

  it("requires subgroup evidence whenever the frozen criterion is subgroup-bound", () => {
    const frozen = protocol();
    const { overallEvidence } = evidencePacket(frozen);
    ...
    ).toThrow(/require subgroup promotion evidence/);
  });

  it("rejects opened final-holdout overall evidence", () => {
    const frozen = protocol();
    const {
      overallEvidence,
      subgroupPromotionEvidence,
    } = evidencePacket(frozen);
    (overallEvidence as any).finalHoldoutAccessed = true;
    ...
    ).toThrow(/sealed-holdout certification-only evidence/);
  });
});
