...
function protocol(): CCFPredictiveValidationProtocol {
  return {
    ...
    protocolId: "rolling-subgroup-baselines-v1",
    frozenAt: "2026-09-18T15:45:00Z",
    ...
    uncertaintyPolicy: {
      ...
      deterministicSeed: "rolling-subgroup-baselines",
...
function execution(frozen: CCFPredictiveValidationProtocol): CCFRollingValidationExecutionV1 {
  ...
  const bindingId = "ccf://rolling-replay-binding/sha256/subgroup-baselines";
  const rows = [
    {
      rowId: "wr-1",
      blockId: "2024-W3",
...
function baselines(
  ...
  return run.predictions.flatMap((prediction) =>
    arms.map((arm) => ({
      ...
      arm,
      predictedFantasyPoints: prediction.actualFantasyPoints - offsets[arm],
      evidenceRef: `ccf://baseline/${prediction.rowId}/${arm}`,
    })),
  );
}

describe("CCF rolling validation subgroup baseline evidence", () => {
  it("derives paired native-baseline evidence within each frozen position subgroup", () => {
    ...
    const evidence = buildCCFRollingValidationSubgroupBaselineEvidence({
      execution: run,
      protocol: frozen,
      baselinePredictions: baselines(run),
      subgroupEvidence,
    });
    ...
    expect(evidence).toMatchObject({
      ...
      certificationOnly: true,
      productionInferenceAuthorized: false,
      subgroupEvidenceRef: subgroupEvidence.evidenceRef,
    });
    expect(evidence.rows).toHaveLength(9);

    const wrUsage = evidence.rows.find(
      (row) => row.subgroup === "WR" && row.arm === "usage_rate",
    );
    expect(wrUsage).toMatchObject({
      totalRows: 2,
      pairedRows: 2,
      independentTimeBlocks: 2,
      minimumSubgroupRows: 2,
      sampleUnderpowered: false,
      candidateMae: 1.5,
      comparatorMae: 3,
      maeImprovement: 1.5,
    });
    expect(wrUsage?.evidenceRef).toMatch(
      /^ccf:\/\/rolling-validation-subgroup-baseline-evidence\/sha256\/[a-f0-9]{64}$/,
    );

    const rbUsage = evidence.rows.find(
      (row) => row.subgroup === "RB" && row.arm === "usage_rate",
    );
    expect(rbUsage).toMatchObject({
      totalRows: 1,
      pairedRows: 1,
      sampleUnderpowered: true,
      comparatorMae: 3,
    });
  });

  it("rejects subgroup evidence from a different replay binding", () => {
    const frozen = protocol();
    const run = execution(frozen);
    const subgroupEvidence = buildCCFRollingValidationSubgroupEvidence({
      execution: run,
    ...
    subgroupEvidence.replayBindingId = "ccf://rolling-replay-binding/sha256/other";
    ...
    ).toThrow(/does not match the validated rolling baseline packet/);
  });

  it("inherits exact baseline coverage checks from the overall baseline contract", () => {
    const frozen = protocol();
    const run = execution(frozen);
    const subgroupEvidence = buildCCFRollingValidationSubgroupEvidence({
      execution: run,
    ...
    const incomplete = baselines(run).slice(1);

    expect(() =>
      buildCCFRollingValidationSubgroupBaselineEvidence({
        execution: run,
        protocol: frozen,
        baselinePredictions: incomplete,
        subgroupEvidence,
      }),
    ).toThrow(/missing historical_mean baseline prediction/);
  });
});
