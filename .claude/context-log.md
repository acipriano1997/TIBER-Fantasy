...
### 2026-09-14 — Codex: Repair Sleeper live smoke contract evaluation
- Fixed the actual PR #24 CI failure: jq evaluated `.data.count` against the leagues array. The corrected comparison retains the root response context.
- Removed Bash 4-only mapfile and GNU-only negative head usage so the smoke runs on stock macOS Bash as well as Linux.
- Added seven actual-shell regression tests for valid/mixed/all-RED portfolios, count/provenance/authority corruption, upstream failures, and negative-query contracts; wired them into Sleeper CI.
- Validation: seven shell regressions passed; full existing Sleeper/health group passed 8 suites / 54 tests with its coverage gates; release build passed; diff check passed. A smaller 3-suite probe passed all 14 assertions but missed the global function-coverage threshold; the complete prescribed group supersedes it.
- Historical run 34547581258 returned a valid live portfolio with 52 RED leagues and zero exact Forecast matches. After this script fix, that genuine model-coverage blocker must still fail. No scoring registry, threshold, or CCF authority was loosened.
